...
def Constructor():
    ...
    Event_10002800()
    Event_10002810()
    GodrickPhaseTwo(0, character=Characters.Godrick)
    GodrickPhaseTwo(1, character=Characters.GodrickClone)
    Event_10002849()
    Event_10002820(0, character=Characters.Godrick, region=10002820, special_effect_id=14790, special_effect_id_1=14791)
    ...
    # Not cloning these, as I think they're more NPC-related.
    Event_10003740(0, Characters.Godrick, 9600, 10009204, 0.5)
    Event_10003741(0, Characters.Godrick, 9601, 10009201, 0.5)
    Event_10003742(0, Characters.Godrick, 9602, 10009202, 0.5)
    Event_10003743(0, Characters.Godrick, 9603, 10009203, 0.5)
    Event_10003744(0, Characters.Godrick, 0, 9101, 10002805, 105.0)
...
def Event_10002800():
    """Event 10002800"""
    EndIfFlagEnabled(Flags.GodrickDead)

    Await(HealthRatioLessThanOrEqual(Characters.Godrick, value=0.0) and HealthRatioLessThanOrEqual(Characters.GodrickClone, value=0.0))
    ...
    IfPlayerInOwnWorld(AND_2)
    IfCharacterDead(AND_2, Characters.Godrick)
    IfCharacterDead(AND_2, Characters.GodrickClone)
    IfCharacterDoesNotHaveSpecialEffect(AND_2, PLAYER, 9646)
    IfConditionTrue(OR_2, input_condition=AND_2)
    IfFlagEnabled(OR_2, Flags.GodrickDead)
    IfConditionTrue(MAIN, input_condition=OR_2)
    KillBossAndDisplayBanner(character=Characters.Godrick, banner_type=BannerType.HollowArenaLoss)
...
def Event_10002810():
    ...
    ForceAnimation(Characters.Godrick, 30000, loop=True, unknown2=1.0)
    ForceAnimation(Characters.GodrickClone, 30000, loop=True, unknown2=1.0)
    ...
    DisableObject(10001870)
    EnableObject(10001871)
    Move(Characters.Godrick, destination=10001895, destination_type=CoordEntityType.Object, model_point=100, short_move=True)
    Move(Characters.GodrickClone, destination=10001895, destination_type=CoordEntityType.Object, model_point=100, short_move=True)
    DisableObject(10001820)
    ForceAnimation(Characters.Godrick, 20000, unknown2=1.0)
    ForceAnimation(Characters.GodrickClone, 20000, unknown2=1.0)
    ...
    EnableBossHealthBar(Characters.Godrick, name=904750000)
    EnableBossHealthBar(Characters.GodrickClone, name=904750010, bar_slot=1)


@RestartOnRest(10002811)
def GodrickPhaseTwo(_, character: uint):
    """Event 10002811"""
    EndIfFlagEnabled(Flags.GodrickDead)
    IfCharacterHasSpecialEffect(OR_1, character, 14785)
    IfFlagEnabled(OR_1, Flags.GodrickDead)
    ...
    WaitFramesAfterCutscene(frames=1)
    EnableFlag(10002802)
    Move(character, destination=10001895, destination_type=CoordEntityType.Object, model_point=100, short_move=True)
...
def Event_10002820(_, character: uint, region: uint, special_effect_id: int, special_effect_id_1: int):
    ...
    Wait(0.10000000149011612)
    IfCharacterOutsideRegion(AND_2, character=character, region=region)
    IfConditionTrue(MAIN, input_condition=AND_2)
    Wait(0.10000000149011612)
...
def MargitDies():
    """Event 10002850"""
    EndIfFlagEnabled(Flags.MargitDead)

    Await(
        HealthValueLessThanOrEqual(Characters.Margit, value=0)
        and HealthValueLessThanOrEqual(Characters.MargitClone, value=0)
    )

    Wait(2.0)
    PlaySoundEffect(Characters.Margit, 77777777, sound_type=SoundType.s_SFX)

    Await(CharacterDead(Characters.Margit) and CharacterDead(Characters.MargitClone))
    Kill(Characters.Margit)
    Kill(Characters.MargitClone)
    KillBossAndDisplayBanner(character=Characters.Margit, banner_type=BannerType.Unknown)
...
def Event_10002860():
    ...
    DisableCharacter(Characters.Margit)
    DisableCharacter(Characters.MargitClone)
    Unknown_2004_73(entity=Characters.Margit, unk_4_8=0)
    Unknown_2004_73(entity=Characters.MargitClone, unk_4_8=0)
    IfPlayerInOwnWorld(AND_1)
    IfEntityWithinDistance(AND_1, entity=Characters.Margit, other_entity=PLAYER, radius=25.0)
    IfConditionTrue(OR_1, input_condition=AND_1)
    IfAttackedWithDamageType(OR_1, attacked_entity=Characters.Margit, attacker=0)
    IfAttackedWithDamageType(OR_1, attacked_entity=Characters.MargitClone, attacker=0)
    IfConditionTrue(AND_10, input_condition=OR_1)
    IfFlagDisabled(AND_10, Flags.MargitDead)
    ...
    ForceAnimation(Characters.Margit, 20000, unknown2=1.0)
    ForceAnimation(Characters.MargitClone, 20000, unknown2=1.0)
    ...
    EnableBossHealthBar(Characters.MargitClone, name=902130010, bar_slot=1)
    SetNetworkFlagState(FlagType.RelativeToThisEventSlot, 0, state=FlagSetting.On)
...
def Event_10002861():
    """Event 10002861"""
    EndIfFlagEnabled(Flags.MargitDead)
    IfCharacterHasSpecialEffect(AND_1, Characters.Margit, 16205)
    IfConditionTrue(MAIN, input_condition=AND_1)
    EnableFlag(10002852)
...
def Event_10002870():
    ...
    IfPlayerInOwnWorld(AND_1)
    IfHealthValueEqual(AND_1, Characters.Margit, value=0)
    IfHealthValueEqual(AND_1, Characters.MargitClone, value=0)
    IfConditionTrue(MAIN, input_condition=AND_1)

    SetBackreadStateAlternate(Characters.Margit, True)
    SetBackreadStateAlternate(Characters.MargitClone, True)
    ...
    SetBackreadStateAlternate(Characters.MargitClone, False)
...
def Event_10003745(_, character: uint):
    ...
    DisableCharacter(character)
    DisableBackread(character)
    EndIfFlagDisabled(Flags.GodrickDead)
    EnableCharacter(character)
    EnableBackread(character)
